...
| Page blocks | `e2e/admin-page-block-crud.spec.ts` | Creates a unique `BLOCK` page, opens its edit form to verify its populated metadata and BLOCK-page information without an HTML content editor, then creates a PageBlock with raw `configJson`, edits title/type/order/visibility/config, deletes the block, and cleans up the page. | Yes, test-created page and block only |
